...
const ComboboxContext = React.createContext<{ searchValue: string }>({
  searchValue: "",
});
...
const ComboboxOuter = ({
  ...
  onInputValueChange,
  ...props
}: ComboboxPrimitive.Root.Props<any>) => {
  ...
  return (
    ...
    >
      <ComboboxContext.Provider value={{ searchValue }}>
        {children}
...
const Combobox = ComboboxOuter as typeof ComboboxPrimitive.Root;
...
function ComboboxItem({
  ...
  const isMatch = React.useMemo(() => {
    ...
    return valueStr.includes(searchStr) || matchText.includes(searchStr);
  }, [searchValue, value, children, textValue]);
...
function ComboboxChips({
  ...
  return (
    <ComboboxPrimitive.Chips
      ...
      className={cn(
        "dark:bg-input/30 border-input focus-within:border-ring focus-within:ring-ring/50 has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive dark:has-aria-invalid:border-destructive/50 flex min-h-9 flex-nowrap overflow-x-auto items-center gap-1.5 rounded-md border bg-transparent bg-clip-padding px-2.5 py-1.5 text-sm shadow-xs transition-[color,box-shadow] focus-within:ring-[3px] has-aria-invalid:ring-[3px] has-data-[slot=combobox-chip]:px-1.5",
        className,
      )}
      {...props}
    />
